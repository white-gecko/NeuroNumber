 ...
 */
package at.neuro.number.neurophstudio;
...
/**
 * 
 * This class is used to draw letters on, it also contains the methods that are
 * used to get the image of an letter
 * 
 * @author Boris Horvat
 */
public class DrawingPanel extends JPanel {
	...
	public BufferedImage getDrawnLetter() throws Exception {
		...
		try {
			...
			leftCoordinate = getLeftCoordinate(upperCoordinate,
					bottomCoordinate);
			rightCoordinate = getRightCoordinate(upperCoordinate,
	...
	public void saveDrawnLetter(File file) throws Exception {
	...
	public File saveDrawnLetter(String fileName) throws Exception {
		try {
			File file = new File(fileName);
			saveDrawnLetter(file);
			return file;
		} catch (Exception ex) {
			throw new RuntimeException(ex.getMessage());
		}
	...
	/**
	 * This method creates new image with new dimensions based on the old image
	...
	private BufferedImage reSizingImage(BufferedImage bufferedImage) {
		...
		BufferedImage newImage = new BufferedImage(FIXED_WIDTH, FIXED_HEIGHT,
				bufferedImage.getType());
		Graphics2D g = newImage.createGraphics();
		...
		g.drawImage(bufferedImage, 0, 0, FIXED_WIDTH, FIXED_HEIGHT, 0, 0,
				bufferedImage.getWidth(), bufferedImage.getHeight(), null);
		g.dispose();

		return newImage;

		// ImageJ2SE neurophImage = (ImageJ2SE)
		// ImageFactory.createImage(newImage.getWidth(), newImage.getHeight(),
		// newImage.getType());
		// neurophImage.setBufferedImage(newImage);
		//
		// return ((ImageJ2SE)ocrUtil.resizeImage(neurophImage, FIXED_WIDTH,
		// FIXED_HEIGHT)).getBufferedImage();
	}
	...
	private int getRightCoordinate(int upperCoordinate, int bottomCoordinate,
			int leftCoordinate) throws Exception {
